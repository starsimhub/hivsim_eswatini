"""Prevalence-target VMMC as an in-repo subclass.

Restores the exp-005 fix that the stisim 1.5.6 VMMC rewrite overwrote. Upstream
`sti.VMMC` now applies coverage as a per-step *hazard* on the uncircumcised pool
and ignores age stratification, so circumcision overshoots to ~100%. This
subclass instead treats coverage as an age-stratified circumcision *prevalence*
(stock) target, matching SHIMS3/PHIA cross-sectional data.

Lives in the repo (not as a patch to the editable stisim checkout) so a future
`git pull` of stisim cannot silently wipe it again — which is exactly what
happened to the original exp-005 patch.

Once validated in exp 015, promote this class into the repo-level
`interventions.py` and use it in `make_interventions()`.
"""
