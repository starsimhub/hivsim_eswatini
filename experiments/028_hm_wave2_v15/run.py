...
MODEL_TAG = "model-v1.5"  # v1.4 + the non-negative CD4 invariant; see 027's SUMMARY
...
def _one_point(idx, row):
    ...
    # --- CD4 guard telemetry (028) ---------------------------------------------
    # model-v1.5 clips nonphysical CD4 at the two positional rate lookups rather
    # than letting them raise IndexError. Every clip is recorded on the disease
    # module. Carry a per-point summary into the parquet so the defect can be
    # quantified across all 1000 points instead of only glimpsed in a log tail.
    #
    # Expected value is zero. A non-zero n_cd4_guard is a model defect to fix
    # upstream, NOT a result to accept — see 027's SUMMARY for the hypothesis
    # these columns are designed to test (on-ART agents with negative dur_art).
    ev = getattr(sim.diseases.hiv, "guard_events", [])
    out["n_cd4_guard"] = len(ev)
    out["n_cd4_clipped"] = int(sum(e["n"] for e in ev)) if ev else 0
    out["cd4_guard_min"] = float(min(e["min_cd4"] for e in ev)) if ev else np.nan
    if ev:
        # Weight the state fractions by how many agents each event clipped, so a
        # single large event is not outvoted by many one-agent events.
        tot = sum(e["n"] for e in ev)
        for k in ("f_on_art", "f_acute", "f_latent", "f_falling",
                  "f_art_discontinued", "f_dur_art_negative"):
            out[f"cd4_guard_{k}"] = sum(e["n"] * e[k] for e in ev) / tot
        out["cd4_guard_min_dur_art"] = float(min(e["min_dur_art"] for e in ev))
    else:
        for k in ("f_on_art", "f_acute", "f_latent", "f_falling",
                  "f_art_discontinued", "f_dur_art_negative"):
            out[f"cd4_guard_{k}"] = np.nan
        out["cd4_guard_min_dur_art"] = np.nan
