"""
Plot network structure figure.

Panels:
    A: Lifetime partner distribution by sex
    ...
    E: Female partnership status by age
...
def plot_lifetime_partners(data, ax):
    """Panel A: Lifetime partner distribution by sex."""
    max_partners = 20
...
def plot_debut_age(data, ax):
    """Panel D: CDF of age at sexual debut by sex."""
    ...
    f50 = np.percentile(f_ages, 50)
    m50 = np.percentile(m_ages, 50)
...
if __name__ == '__main__':
    ...
    plot_condom_use(fig.add_subplot(gs[1, 2]))

    for i, ax in enumerate(fig.axes):
        ax.text(-0.10, 1.06, chr(65 + i), transform=ax.transAxes,
                fontsize=28, fontweight='bold', va='top')

    pl.savefig(f'{FIGURES_DIR}/fig3_network.png', dpi=200, bbox_inches='tight')
    print(f'Saved {FIGURES_DIR}/fig3_network.png')
